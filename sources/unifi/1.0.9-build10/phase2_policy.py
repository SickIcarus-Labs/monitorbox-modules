...
def _device_uplink_port_roles(raw_devices: Any) -> dict[tuple[str, int], str]:
    """Fallback projection from legacy stat/device child uplink tuples."""
...
def _legacy_topology_port_roles(
    raw_devices: Any,
    raw_topology: Any,
) -> dict[tuple[str, int], str]:
    """Project authoritative wired device-to-device edges from legacy /topology.

    Broad Leaf census on Network 10.6.101 established the provider shape as:
    ``downlinkMac/downlinkPortNumber`` for the child side and
    ``uplinkMac/uplinkPortNumber`` for the parent side. Only explicit WIRED
    edges between two known UniFi devices with both physical port identifiers
    are eligible. Client edges without a downlink port remain ordinary/unknown.
    """
    if not isinstance(raw_devices, list) or not isinstance(raw_topology, Mapping):
        return {}
    ...
    for edge in edges:
        if not isinstance(edge, Mapping):
            continue
        if str(edge.get("type") or "").strip().casefold() != "wired":
            continue
        ...
        if (
            child_mac not in known_macs
            or parent_mac not in known_macs
            or child_idx is None
...
def _topology_port_roles(
    raw_devices: Any,
    raw_topology: Any = None,
) -> dict[tuple[str, int], str]:
    """Use legacy /topology as primary authority; stat/device is safe fallback.

    A non-empty authoritative topology snapshot wins as a whole so stale or
    differently-shaped child uplink tuples cannot create a second conflicting
    recommendation. If /topology is unavailable or yields no eligible physical
    device-to-device edges, retain the pre-existing provider-native uplink tuple
    behavior as a fail-closed fallback rather than inventing heuristics.
    """
    topology_roles = _legacy_topology_port_roles(raw_devices, raw_topology)
    if topology_roles:
        return topology_roles
    return _device_uplink_port_roles(raw_devices)
...
def _enrich_port_roles(
    ...
    topology_roles = _topology_port_roles(raw_devices, raw_topology)
    for port in ports:
...
class UniFiPhase2RuntimeExecutor(_BaseUniFiRuntimeExecutor):
    """Reproject authenticated inventory through expectation-aware discovery."""
    ...
    async def execute(
        ...
        raw_topology: Mapping[str, Any] | None = None
        if metadata.get("source_available") is True:
            ...
            try:
                topology = await self._get(
                    request.options,
                    f"/proxy/network/v2/api/site/{site}/topology",
                )
                if isinstance(topology, Mapping):
                    raw_topology = topology
            except Exception as exc:
                metadata["topology_error"] = f"{type(exc).__name__}: {exc}"[:300]

        topology_roles = _topology_port_roles(raw_devices, raw_topology)
        metadata["topology_authority"] = (
            "legacy_topology"
            if _legacy_topology_port_roles(raw_devices, raw_topology)
            else "stat_device_uplink"
            if topology_roles
            else "unavailable"
        )
...
__all__ = [
    ...
    "_device_uplink_port_roles",
    "_enrich_port_roles",
    "_explicit_port_role",
    "_inventory_authority",
    "_legacy_topology_port_roles",
    "_port_index",
