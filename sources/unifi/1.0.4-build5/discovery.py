...
def wan_interface_addresses(raw_health: Any) -> frozenset[str]:
    ...
    provider rows explicitly describing the WAN subsystem can suppress a
    discovery Ping recommendation.
    """
...
def unifi_evidence(
    ...
    """Project authenticated UniFi inventory into provider-owned discovery truth."""
    ...
    if isinstance(raw_devices, list):
        for device in raw_devices:
            ...
            wan_interface = bool(ip and ip in wan_addresses)
            addresses = () if wan_interface or not ip else (ip,)
            ...
            if wan_interface and ip:
                metadata.update(
                    {
                        "wan_interface_address": ip,
                        "wan_interface_informational": True,
                        "reachability_recommendation_suppressed": True,
                        "reachability_suppression_reason": "wan_interface_address",
                    }
                )
            result.append(
                DiscoveryEvidence(
