...
class UniFiRuntimeExecutor(_VerticalUniFiRuntimeExecutor):
    """UniFi executor with provider-owned recovery and bounded auth retries.

    UniFi authentication/session state is ephemeral observer state. Rejected
    cached sessions are discarded, but failed logins are also rate limited
    locally so concurrent inventory fan-out and fast monitoring cadence cannot
    keep a controller-side login lockout alive.
    """
    ...
    def _record_auth_cooldown(
        ...
        *,
        ...
        initial = (
            _RATE_LIMIT_INITIAL_BACKOFF_SECONDS
            if status == 429
            else _AUTH_DENIAL_INITIAL_BACKOFF_SECONDS
        )
        requested = self._retry_after_seconds(retry_after) if status == 429 else None
        if requested is not None:
            delay = requested
        elif previous is not None:
            delay = max(initial, previous[1])
        else:
            delay = initial
        next_delay = min(_MAX_AUTH_BACKOFF_SECONDS, max(initial, delay * 2.0))
