...
def _topology_port_roles(raw_devices: Any) -> dict[tuple[str, int], str]:
    ...
    The legacy authenticated device payload already exposes ``uplink.port_idx``,
    ``uplink.uplink_mac`` and ``uplink.uplink_remote_port``; the base runtime
    uses the same tuple for durable link-speed expectations. This is provider
    topology evidence, not a switch-name or port-number heuristic.
    """
    ...
    for child in raw_devices:
        ...
        child_idx = uplink.get("port_idx")
        if isinstance(child_idx, int) and not isinstance(child_idx, bool) and child_idx >= 0:
            roles[(child_mac, child_idx)] = "uplink"

        parent_mac = str(uplink.get("uplink_mac") or "").strip().casefold()
        parent_idx = uplink.get("uplink_remote_port")
        if (
            parent_mac in known_macs
            and isinstance(parent_idx, int)
            and not isinstance(parent_idx, bool)
            and parent_idx >= 0
        ):
            # The parent-side port is an inter-device infrastructure link even
            # though it is directionally a downlink from the parent.
...
class UniFiPhase2RuntimeExecutor(_BaseUniFiRuntimeExecutor):
    ...
    async def execute(
        ...
        # Informational WAN context stays in provider inventory/status but does
        # not enter actionable generic discovery evidence.
