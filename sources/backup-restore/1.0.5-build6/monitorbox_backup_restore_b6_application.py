"""Operator-facing Backup / Restore build-5 product surface."""
...
<section class="card"><h2>Configuration snapshots</h2><p class="muted">Retained configuration revisions capture canonical monitoring authority and versioned snapshot-capable module preferences, including dashboard layouts when recorded. Historical pre-layout revisions retain the current dashboard layout and report that explicitly. Full appliance backups remain the complete recovery contract.</p><div class="row"><button id="configExport" class="button">Download config recovery bundle</button><input id="bundle" type="file"><button id="configRestore" class="button danger">Validate &amp; restore config</button></div><div id="configState" class="status" style="margin-top:12px">No configuration restore requested.</div></section>
<div class="grid"><section class="card"><h2>Config doctor</h2><div id="doctor" class="status">Not run yet.</div></section><section class="card"><h2>Retained revisions</h2><p class="muted">A retained revision restores canonical monitoring configuration and any paired module-owned snapshot preferences recorded at that revision. Older snapshots retain missing preferences from the current installation and display a warning. MonitorBox validates protected material, commits a <strong>new</strong> canonical revision and never rewinds history.</p><div id="revisionState" class="status">No retained revision restore requested.</div><div id="preferencesNotice" class="status warn hidden" role="alert" aria-live="assertive" style="margin-top:12px"><span id="preferencesNoticeText"></span> <button id="dismissPreferenceNotice" type="button" class="button" style="margin-left:8px">Dismiss</button></div><div id="revisions" style="margin-top:8px" class="muted">Loading…</div></section></div>
...
<dialog id="revisionDialog"><h2>Restore retained revision?</h2><div id="revisionPreview" class="preview"></div><p class="warning"><strong>This changes active canonical configuration.</strong><br>The selected retained snapshot will be validated against current protected material and committed as a <strong>new</strong> revision. Existing revision/generation history is not rewound. Runtime will reconcile and the controller may restart.</p><div class="row" style="justify-content:flex-end"><button id="cancelRevisionRestore" class="button">Cancel</button><button id="confirmRevisionRestore" class="button danger">Restore as new revision</button></div></dialog>
<script>
const $=id=>document.getElementById(id);let csrf=null,currentPolicy=null,restoreToken=null,revisionCandidate=null;const API='/api/v2/config/backup-restore';
const RESTORE_NOTICE_KEY='monitorbox.br6.snapshotNotice';
function showPreferenceNotice(data){
  const message=typeof data?.preferences_notice==='string'
    ?data.preferences_notice.slice(0,2048):'';
  const notice=$('preferencesNotice');
  $('preferencesNoticeText').textContent=message;
  notice.classList.toggle('hidden',!message);
  try{if(message)sessionStorage.setItem(RESTORE_NOTICE_KEY,JSON.stringify({
    message,expires:Date.now()+10*60*1000
  }));else sessionStorage.removeItem(RESTORE_NOTICE_KEY)}catch(_){}
}
function restoreSavedNotice(){
  try{
    const record=JSON.parse(sessionStorage.getItem(RESTORE_NOTICE_KEY)||'null');
    if(record&&typeof record.message==='string'&&record.expires>Date.now()){
      $('preferencesNoticeText').textContent=record.message.slice(0,2048);
      $('preferencesNotice').classList.remove('hidden');
    }else sessionStorage.removeItem(RESTORE_NOTICE_KEY);
  }catch(_){}
}
$('dismissPreferenceNotice').onclick=()=>showPreferenceNotice({});

function showAuthenticated(){$('authState').classList.remove('hidden');$('login').classList.add('hidden');$('app').classList.remove('hidden');restoreSavedNotice()}
...
function renderRevisions(items){const root=$('revisions');root.replaceChildren();if(!items.length){root.className='muted';root.textContent='No retained revisions yet.';return}root.className='';for(const item of items){const row=document.createElement('div');row.className='revision'+(item.valid?'':' invalid');const info=document.createElement('div');const title=document.createElement('strong');title.textContent=item.valid?`Revision ${item.revision}`:`Invalid retained revision`;const sub=document.createElement('div');sub.className='muted';sub.textContent=item.valid?`${item.name} · ${String(item.content_hash||'').slice(0,16)}`:`${item.name} · ${item.error||'validation failed'}`;info.append(title,sub);row.append(info);if(item.valid){const button=document.createElement('button');button.className='button danger';button.textContent='Restore as new revision';button.onclick=()=>showRevisionRestore(item);row.append(button)}root.append(row)}}
async function refresh(){try{const [vault,doctor,revisions,policy,schedule,restore]=await Promise.all([jsonApi(`${API}/backups`),jsonApi('/api/v2/config/doctor'),jsonApi('/api/v2/config/revisions'),jsonApi(`${API}/policy`),jsonApi(`${API}/schedule`),jsonApi(`${API}/restore/status`)]);renderBackups(vault.backups||[]);renderPolicy(policy.policy,schedule);$('doctor').className='status '+(doctor.status==='healthy'?'good':'');$('doctor').textContent=`${String(doctor.status).toUpperCase()} · ${doctor.errors} error(s), ${doctor.warnings} warning(s)`;renderRevisions(revisions.revisions||[]);if(restore.last_result){const r=restore.last_result;$('backupState').className='status '+(r.success?'good':'bad');$('backupState').textContent=r.success?`Last appliance restore completed successfully · revision ${r.canonical_revision}.`:`Last appliance restore failed safely · ${r.error||'prior authority preserved'}.`}}catch(e){$('backupState').className='status bad';$('backupState').textContent=e.message}}
...
function showRevisionRestore(item){revisionCandidate=item;$('revisionPreview').textContent=`Source canonical revision: ${item.revision}\nRetained basename: ${item.name}\n\nMonitorBox will validate this retained snapshot and commit it as a new canonical revision. Existing revision history will remain intact.`;$('revisionDialog').showModal()}
$('cancelRevisionRestore').onclick=()=>{$('revisionDialog').close();revisionCandidate=null};
$('confirmRevisionRestore').onclick=async()=>{if(!revisionCandidate)return;const item=revisionCandidate,state=$('revisionState'),button=$('confirmRevisionRestore');button.disabled=true;try{state.className='status';state.textContent=`Validating retained revision ${item.revision} (${item.name})…`;const data=await jsonApi(`/api/v2/config/revisions/${encodeURIComponent(item.name)}/restore`,{method:'POST'});$('revisionDialog').close();revisionCandidate=null;showPreferenceNotice(data);state.className='status '+(data.preferences_notice?'warn':'good');state.textContent=`Restored source revision ${data.source_revision} as new revision ${data.applied_revision}. Runtime generation ${data.runtime_generation}; ${data.runtime_reconcile}.`+(data.preferences_notice?'\n\n'+data.preferences_notice:'');setTimeout(()=>location.reload(),7000)}catch(e){state.className='status bad';state.textContent=e.message}finally{button.disabled=false}};
...
$('configRestore').onclick=async()=>{const file=$('bundle').files[0],state=$('configState');if(!file){state.textContent='Choose a configuration recovery bundle first.';return}if(!confirm(`Restore configuration from ${file.name}?`))return;try{const r=await fetch('/api/v2/config/recovery/restore',{method:'POST',headers:{'X-MonitorBox-CSRF':csrf,'Content-Type':'application/octet-stream'},body:file});const text=await r.text();let data={};try{data=JSON.parse(text)}catch{}if(!r.ok)throw new Error(data.error||text||`HTTP ${r.status}`);showPreferenceNotice(data);state.className='status '+(data.preferences_notice?'warn':'good');state.textContent=`Restored source revision ${data.source_revision} as ${data.applied_revision}; controller restart scheduled.`+(data.preferences_notice?'\n\n'+data.preferences_notice:'')}catch(e){state.className='status bad';state.textContent=e.message}};
authenticate();
</script></body></html>'''
