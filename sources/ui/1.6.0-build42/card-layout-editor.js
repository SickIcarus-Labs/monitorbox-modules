...
(()=>{
  ...
  function renderAvailable(){
    ...
    const advanced=$('showDiscovered').checked;
    const grouped=missing.filter(item=>item.kind==='host'&&!item.defaultVisible);
    $('discoveredCount').textContent=grouped.length
      ?grouped.length+' discovered device(s) remain in their monitored families. '+(advanced?'You can explicitly promote one below.':'Enable Advanced to promote one individually.')
      :'All discovered devices are already represented or selected.';
    let count=0;
    for(const item of missing){
      if(!advanced&&item.kind==='host'&&!item.defaultVisible)continue;
    ...
    $('add').disabled=pending||!(advanced?missing.length:missing.length-grouped.length);
  }
  function render(){
    $('editor').hidden=false;
  ...
  $('saveContents').onclick=()=>{
    ...
    const preference={schema_version:1};
    if(title!==base)preference.title=title;
  ...
  $('showDiscovered').onchange=()=>renderAvailable();
  $('add').onclick=()=>{
