...
(()=>{
  ...
  function installParityMaintenance(){
    if(typeof parityCoreCard!=='function' || parityCoreCard?.phase6Build22)return false;
  ...
  function relationGraph(model){
    ...
    return {bySystem:new Map(),byConnection:new Map()};
  }
  ...
  function connectionRow(path){
    const connection=path.connection||path;
    const id=String(connection.id||'');
    const label=connection.label||connection.adapter||id;
  ...
  function systemRow(system,graph){
    ...
    return `<div class="phase6-advanced-system" data-phase6-system="${esc(id)}">${rowButton('system',id,system.label||id,kindLabel(system.kind))}${paths.length?`<div class="phase6-advanced-system-paths"><span class="phase6-advanced-path-label">Connections</span>${paths.map(connectionRow).join('')}</div>`:''}</div>`;
  }
  ...
  function buildAdvancedMarkup(model,term=''){
    ...
    const connectionRows=connections.map(connection=>graph.byConnection?.get(String(connection.id||''))||{connection,objects:[]}).filter(path=>matches(path.connection)||path.objects.some(matches));
    if(connectionRows.length||!needle)sections.push(groupMarkup('Connections',connections.length,connectionRows.map(connectionRow).join('')||'<div class="tree-empty">No Connections match this search.</div>','connections'));
  ...
  function installAdvanced(){
    ...
    if(typeof load==='function'&&!load.phase6Build22){
      const baseLoad=load;
  ...
  globalThis.MonitorBoxPhase6Physical=Object.freeze({
    advancedSummary,buildAdvancedMarkup,installParityMaintenance,kindLabel,renderAdvanced,
  });
