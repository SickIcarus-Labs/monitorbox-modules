  ...
  async function moduleRemovalReviewRequest(moduleId) {
    const response = await fetch(`/api/v2/config/modules/${moduleId}/remove/review`, {
      method: "POST",
    ...
    if (response.status === 404 || response.status === 405) {
      throw new Error(
        "Safe module-removal review is unavailable on this Core. Update Core before removing modules."
      );
    }
    if (!response.ok) {
      ...
      throw new Error(message || "Module removal review failed");
    }
    if (
      !payload ||
      ...
      throw new Error("Core returned an invalid module-removal review response.");
    }
    return payload;
  }
  ...
  async function mutateModule(module, action) {
    ...
    if (action === "rollback" && !window.confirm(
      `Roll back ${module.display_name || module.module_id}?`
    )) return;
    ...
    try {
      ...
      let payload;
      if (action === "remove") {
        const review = await moduleRemovalReviewRequest(moduleId);
        if (review.review_required === true) {
          const adminPassword = await confirmReviewedModuleRemoval(module, review);
          ...
          payload = await jsonRequest(`/api/v2/config/modules/${moduleId}/remove/apply`, {
            ...
            body: JSON.stringify({ plan_id: review.plan_id, admin_password: adminPassword }),
          });
        } else {
          if (review.safe_simple_remove !== true) {
            throw new Error("Core did not authorize a simple unreferenced module removal.");
          }
          if (!window.confirm(
            `Remove ${module.display_name || module.module_id}? This module has no canonical monitoring references.`
          )) {
            setStatus("Module removal cancelled.");
            return;
          }
          payload = await jsonRequest(`/api/v2/config/modules/${moduleId}/remove`, {
            method: "POST",
