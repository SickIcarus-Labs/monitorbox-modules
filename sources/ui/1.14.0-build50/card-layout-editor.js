...
(()=>{
  ...
  const frameCss=[
    ...
    '.mb-arrange-handle,.mb-arrange-menu-button{position:absolute;z-index:7;top:-6px;',
      'border:1px solid #607fa1;border-radius:9px;background:#18324b;color:#f4f8fb;',
      'min-height:42px;min-width:42px;font-size:17px;touch-action:none;cursor:grab}',
    '.mb-arrange-handle{left:-6px}.mb-arrange-menu-button{right:-6px;cursor:pointer;touch-action:manipulation}',
    '.mb-arrange-frame-card .mb-arrange-actions{position:absolute;z-index:8;',
      'right:0;top:40px;display:none;gap:3px;flex-wrap:wrap;padding:5px;',
      'border:1px solid #607fa1;border-radius:9px;background:#101e31;',
      'box-shadow:0 5px 18px #0008;max-width:100%}',
    '.mb-arrange-frame-card .mb-arrange-actions[data-open=true]{display:flex}',
  ...
  function decorateFrame(){
    ...
    for(const [lane,column] of columns.entries()){
      ...
      for(const card of [...column.children]){
        ...
        handle.textContent='☷';handle.title='Drag to arrange; arrow keys also move';
        ...
        menu.type='button';menu.className='mb-arrange-menu-button';
        menu.textContent='Move';menu.setAttribute('aria-label','Move '+label(saved));
        const actions=doc.createElement('div');actions.className='mb-arrange-actions';
        menu.onclick=()=>{actions.dataset.open=actions.dataset.open==='true'?'false':'true';};
        ...
        actions.append(
          ...
          select);
        wrapper.append(handle,menu,actions);
      }
    ...
    $('arrangeStatus').textContent=$('arrangeStatus').textContent||
      'Actual homepage cards; move handles and menus only appear in this editor.';
    frameHeight();
    const grid=doc.querySelector('#core-grid');
  ...
  function renderArrangement(){
    ...
    $('arrangeDescription').textContent='Live homepage preview · '+visible.length+
      ...
      '. Drag a handle onto the actual card or choose Move. All card content, dimensions and typography are from the homepage.';
  ...
  function render(){
    ...
    $('rebuildBootstrap').disabled=arranging||pending;
  ...
  // One explicit, backed-up, optimistic-concurrency configuration revision:
  // this intentionally discards ALL old card contents and positions, even
  // custom cards. It does not alter monitoring, graph widgets or other modules.
  $('rebuildBootstrap').onclick=()=>{
    if(pending||arranging)return;
    const prior=Object.values(working?.data?.sites||{}).flatMap(s=>s.cards||[]);
    const fresh=publicSites.reduce((sum,site)=>sum+policy.defaults(site,site.objects||[]).length,0);
    if(!confirm('Rebuild ALL sites from clean bootstrap? This replaces '+prior.length+
      ' existing card definitions, custom cards and every saved position with '+fresh+
      ' freshly discovered defaults. Your existing configuration snapshots are not deleted. Make a full appliance backup before Apply.'))return;
    const sites={};
    for(const site of publicSites)sites[site.id]={mode:'auto',
      cards:policy.defaults(site,site.objects||[])};
    const reset={schema_version:policy.SCHEMA,data:{sites}};
    try{policy.validate(reset);working=reset;
      notice('Clean-bootstrap reset staged for ALL sites. Previous contents and placements are discarded in the draft. Validate, preview and Apply once to create a recoverable snapshot revision.');
      render();
    }catch(error){notice('Rebuild refused: '+error.message,true);}
  };
  $('reset').onclick=()=>{
    working.schema_version=policy.SCHEMA;
