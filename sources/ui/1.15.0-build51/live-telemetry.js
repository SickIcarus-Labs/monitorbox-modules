...
async function refreshLiveTelemetry(){
  // Interest belongs to the foreground viewer. A hidden browser tab should
  // allow its six-second Core lease to expire and restore background cadence.
  if(document.hidden||liveTelemetryRefreshRunning)return;
  liveTelemetryRefreshRunning=true;
  const selectedDetail=app.selected
    ?app.selected.siteId+'/'+app.selected.objectId:'';
  const trafficDetail=app.liveTrafficSelection
    ?app.liveTrafficSelection.siteId+'/'+app.liveTrafficSelection.objectId:'';
  const details=new Set();
  // Preserve the original drawer detail before bounded card interests.
  for(const detail of [selectedDetail,trafficDetail,
      ...(globalThis.MonitorBoxCardComposer?.visibleLiveDetails?.()||[])]){
    if(typeof detail==='string'&&detail&&detail.length<=256&&
        detail.indexOf('/')>0)details.add(detail);
    if(details.size>=32)break;
  }
  const windowSeconds=selectedDetail||trafficDetail?900:300;
  const query=new URLSearchParams({window:String(windowSeconds)});
  for(const detail of details)query.append('detail',detail);
  try{
    app.liveTelemetry=await api('/api/v2/live?'+query.toString(),{
      headers:{'X-MonitorBox-Viewer':app.liveTelemetryViewer}
    });
...
document.addEventListener('visibilitychange',()=>{
  if(!document.hidden)void refreshLiveTelemetry();
});
