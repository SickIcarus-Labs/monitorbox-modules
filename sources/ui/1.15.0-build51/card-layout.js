...
(()=>{
  ...
  const state={loaded:false,blocked:false,entry:null,error:null,pinned:false,preview:null};
  const activeEntry=()=>state.preview||state.entry;
  const activePinned=()=>state.preview?false:state.pinned;
  ...
  function effectiveCards(site){
    ...
    return policy.visible(site,objects(site),activeEntry(),activePinned());
  }
  function project(site){return effectiveCards(site).map(row=>row.value);}
  function spatialFor(site){
    const current=activeEntry();
    if(!state.loaded||state.blocked||current?.schema_version!==4)return null;
    const saved=current?.data?.sites?.[site.id];
    if(!saved?.arranged||!(saved.mode==='custom'||(activePinned()&&saved.mode==='auto')))return null;
  ...
  function displayFor(site,id){
    if(!state.loaded||state.blocked)return null;
    // A fresh post-bootstrap site uses the SAME curated defaults as the
    // regenerate action, while a saved v4 card layout remains exact until
    // its operator explicitly regenerates it. Restored snapshots never
    // acquire unsolicited CPU/memory rows.
    const rows=policy.siteRows(activeEntry(),site,objects(site),activePinned());
  ...
  // Same-origin editor iframe may stage an opaque draft for *rendering only*.
  // It never writes canonical preferences or changes the real homepage.
  function setPreview(entry){
    state.preview=policy.validate(entry);
    state.loaded=true;state.blocked=false;state.error=null;
    refreshGrid();
  }
  ...
  globalThis.MonitorBoxCardLayout=Object.freeze({
    ...
    effectiveCards,displayFor,project,spatialFor,load,refreshGrid,setPreview,state:()=>({...state}),
  });
})();
