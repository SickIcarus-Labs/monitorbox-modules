...
from ...plugin_api import AddObjectIntent, ConnectionPlan
...
def _integration_provider(
    ...
    if (
        ...
        raise web.HTTPBadRequest(
            text="portainer inventory Connection has no unambiguous owning System"
        )
    return agent_id, provider, connection_id, dependencies[0]
...
def _workload_object(
    ...
    agent_id, inventory_provider, _, _ = _integration_provider(working, site_id)
...
class PortainerCandidateAdoption:
    ...
    def plan_candidate(
        ...
        *,
        ...
        if (
            getattr(candidate, "kind", None) != "service"
            or _candidate_source(candidate, "portainer") is None
        ):
            raise web.HTTPBadRequest(
                text="Portainer discovery adoption requires Portainer service evidence"
            )
        selected = _selected_capabilities(candidate, capabilities)
        working = copy.deepcopy(dict(context.current_config))
        site = _site(working, context.site_id)
        existing_ids = {
            str(obj.get("id"))
        ...
        _, _, connection_object_id, system_id = _integration_provider(
            working,
            context.site_id,
        )
