...
def generic_workload_evidence(workloads: Any, *, authoritative: bool) -> list[dict[str, Any]]:
    ...
    Every emitted row was returned by a successful authenticated Portainer
    environment inventory and is therefore already under active provider-backed
    observation. ``monitoring_coverage`` communicates that generic fact without
    claiming that a canonical Docker-workload Object already exists. Consumers
    can distinguish current coverage from an optional adoption/configuration
    action without knowing the Portainer provider schema.
    """
    if not isinstance(workloads, list):
        return []
    result: list[dict[str, Any]] = []
