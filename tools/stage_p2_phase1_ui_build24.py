#!/usr/bin/env python3
"""Idempotently stage the P2 Phase-1 UI v1.1.14 build-24 candidate."""
...
RELEASE=("com.sickicarus.monitorbox.ui","1.1.14",24)
ENTRY={
  "manifest":{
    ...
    "state_schema":1,
    "module_type":"ui",
    "entrypoints":{"webui":"monitorbox_ui_b24:install"},
  ...
  "package":"com.sickicarus.monitorbox.ui-1.1.14-build24.zip"
}
...
def stage(path:Path)->bool:
    ...
    if existing:
        if len(existing)!=1 or existing[0]!=ENTRY: raise SystemExit(f"catalog release conflicts with P2 Phase-1 contract: {RELEASE}")
        print("P2 Phase-1 UI candidate already staged"); return False
    ...
    print("staged P2 Phase-1 UI 1.1.14 build 24 after accepted build 23")
    return True
