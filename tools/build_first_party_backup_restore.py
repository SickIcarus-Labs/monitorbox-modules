#!/usr/bin/env python3
"""Build Backup / Restore 1.0.0 build 1 for MonitorBox 2.3 Core."""
...
SOURCE_BLOB = "aba0a115c580bb3c74de651c96762b5577a6c698"
...
def _source(root: Path) -> bytes:
    source_root = root / "sources" / "backup-restore" / "1.0.0-build1"
    actual_names = {path.name for path in source_root.iterdir() if path.is_file()}
    if actual_names != {SOURCE_NAME}:
        raise SystemExit(
            "Backup / Restore 1.0.0 build 1 source shape changed: "
    ...
    if actual_blob != SOURCE_BLOB:
        raise SystemExit(
            "Backup / Restore 1.0.0 build 1 source drift: "
