#!/usr/bin/env python3
"""#104 exact UI50 candidate: real accepted Core 2.6.0, managed UI, Chromium and retained revisions.
...
import build_first_party_ui_build50 as ui_builder  # noqa:E402
import stage_104_ui_build50 as ui_stage  # noqa:E402
...
def archive():
    ...
    meta={**ui_stage.ENTRY["manifest"],"version":"1.14.0","build":50,
          "entrypoints":{"webui":"monitorbox_ui_b50:install"}}
    ...
    artifact=ManagedArtifact(
        ...
        artifact_filename=ui_builder.RELEASE50.filename,
    )
    ...
    def start(self):
        ...
        def run():
            ...
            async def boot():
                ...
                assert installed.active.manifest.build==50
                app=web.Application()
                ...
                assert (activation.version,activation.build)==("1.14.0",50)
                assert any(
...
def browser_contract(server: PairedServer):
    """Install real managed UI50 on real Core and round-trip real revisions.
    ...
    print("UI50 paired real Core: synthetic legacy custom contents/placement staged")
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            page.locator("#rebuildBootstrap").click()
            ...
            assert all("presentation" not in row and "placement" not in row
                       for row in site["cards"])
            ...
            home.close()
            context.close()
            print("UI50 real Core 2.6.0: managed loader, one-revision "
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="monitorbox-ui50-paired-") as temp:
        root = Path(temp)
        prepare(root)
        ...
        try:
            ...
            server.stop()
    print("UI50 exact package sha256=" + digest)
