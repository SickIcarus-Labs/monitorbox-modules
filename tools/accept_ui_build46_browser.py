#!/usr/bin/env python3
"""Disposable UI46 iPad/desktop picker and genuine one-second DOM repaint tests."""
...
fixture_server.fixture_site=site46
...
async def editor_accept(browser,base,fixture,width,height):
    ...
    await context.close()
    print(f"UI46 live grouped picker, iPad/desktop v2→v3 and full A/B selected-item restoration: PASS {width}x{height}")
...
async def live_card_accept(browser,width,height):
    ...
    await page.evaluate("""({site,series})=>{
      ...
      bindCards();
    }""",{"site":site46(),"series":live_series()})
    ...
    assert not errors,errors
    print(f"UI46 iPad/desktop 24-item natural-height columns, exact NIC, "
