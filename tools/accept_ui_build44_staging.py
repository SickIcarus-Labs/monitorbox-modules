#!/usr/bin/env python3
"""UI44 additive staged release and exact UI42 immutable signed predecessor."""
...
import stage_94_ui_build44 as stage
...
def main()->None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert all(row["manifest"]["build"]!=43
                   for row in result
                   if row["manifest"]["module_id"]==MODULE)
        assert stage.stage(path) is False
        assert json.loads(path.read_text())["modules"]==result
    print("UI44: exact accepted UI42 digest, unchanged cumulative source, "
          "one staged UI44 dev supersession and idempotent staging: PASS")
