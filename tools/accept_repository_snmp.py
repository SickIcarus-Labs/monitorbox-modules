#!/usr/bin/env python3
"""Extend first-party repository acceptance through SNMP v1.0.0 build 1."""
...
SNMP_RELEASE = (SNMP_ID, "1.0.0", 1)
CURRENT_RELEASES = set(previous.CURRENT_RELEASES) | {SNMP_RELEASE}
...
def _accept_snmp(root: Path, source: dict) -> None:
    ...
    if manifest != expected_manifest:
        raise AssertionError(f"SNMP v1.0.0 build 1 manifest changed: {manifest!r}")
    ...
    if package_path.name != expected_filename or not package_path.is_file():
        raise AssertionError("SNMP v1.0.0 build 1 generated package is missing or misnamed")
    ...
    if missing:
        raise AssertionError(f"SNMP managed package root omitted contract markers: {missing}")
...
def _package_shape(root: Path, source: dict) -> None:
    ...
    prior["modules"] = [
        ...
        if stable._release_identity(item) != SNMP_RELEASE
    ]
