#!/usr/bin/env python3
"""Extend the standalone managed UI chain through v1.1.7 build 15.

Build 15 supersedes physically rejected dev-only build 14. It preserves all
accepted Phase-2 behavior while adding two late-loading repairs: provider-level
Advanced transaction isolation against passive render drift (#164), and safe
canonical browser-URL extraction from explicit provider configuration (#206).
...
import build_first_party_ui_build14 as previous

UI_VERSION = "1.1.7"
UI_BUILD = 15
RELEASE15 = stable.Release(
    build=UI_BUILD,
    certified_sha="phase2-physical-repairs-2",
    version=UI_VERSION,
)
SOURCE_FILES = frozenset(("phase2-physical-repairs2.js",))


def _build15_assets(root: Path) -> dict[str, bytes]:
    assets = previous._build14_assets(root)
    source_root = root / "sources" / "ui" / "1.1.7-build15"
    actual = {path.name for path in source_root.iterdir() if path.is_file()}
    if actual != SOURCE_FILES:
        raise SystemExit(
            "UI 1.1.7 build-15 delta shape changed: "
            f"missing={sorted(SOURCE_FILES-actual)}, extra={sorted(actual-SOURCE_FILES)}"
        )
    assets["phase2-physical-repairs2.js"] = (
        source_root / "phase2-physical-repairs2.js"
    ).read_bytes()
    return assets
...
def _standalone_application() -> bytes:
    ...
    for old, new in replacements:
        if payload.count(old) != 1:
            raise SystemExit(f"UI build-15 standalone seam changed: {old[:100]!r}")
...
def _package_files(root: Path, release: stable.Release) -> dict[str, bytes]:
    ...
    for name, payload in _build15_assets(root).items():
    ...
    if offenders:
        raise SystemExit(
            "standalone UI build 15 unexpectedly references retired Core UI authority: "
