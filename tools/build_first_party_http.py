#!/usr/bin/env python3
"""Build the independently managed HTTP integration for MonitorBox 2.3 Core."""
...
MODULE_BUILD = 1
IMPORT_PACKAGE = "monitorbox_http_b1"
FILENAME = f"{MODULE_ID}-{MODULE_VERSION}-build{MODULE_BUILD}.zip"

SOURCE_BLOBS = {
    "__init__.py": "afaab94b284d6814f64017bb9a62f1f96d4ef9de",
}
...
def _source_files(root: Path) -> dict[str, bytes]:
    source_root = root / "sources" / "http" / "1.0.0-build1"
    ...
    if actual_names != expected_names:
        ...
        raise SystemExit(
            f"HTTP 1.0.0 build 1 source shape changed: missing={missing}, extra={extra}"
        )
    ...
    for name, expected_blob in SOURCE_BLOBS.items():
        ...
        if actual_blob != expected_blob:
            raise SystemExit(
                f"HTTP 1.0.0 build 1 source drift for {name}: "
...
def _rewrite_source(name: str, payload: bytes) -> bytes:
    ...
    for old, new in _CORE_IMPORT_REWRITES:
        if text.count(old) != 1:
            raise SystemExit(f"HTTP managed import contract changed for {old!r} in {name}")
        text = text.replace(old, new, 1)

    if name == "__init__.py":
        bundled_entrypoint = 'entrypoints={"integration": "monitorbox.v2.integrations.http:PLUGIN"}'
        managed_entrypoint = f'entrypoints={{"integration": "{IMPORT_PACKAGE}:PLUGIN"}}'
        if text.count(bundled_entrypoint) != 1:
            raise SystemExit("HTTP package root bundled entrypoint contract changed")
        ...
        if text.count(old_core_requirement) != 1:
            raise SystemExit("HTTP package root Core requirement contract changed")
    ...
    if remaining:
        raise SystemExit(f"HTTP managed namespace rewrite incomplete in {name}: {remaining}")
