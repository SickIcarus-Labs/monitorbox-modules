#!/usr/bin/env python3
"""Build UniFi Network v1.0.6 build 7 over immutable build-6 dev history."""
...
import build_first_party_unifi_b6 as previous

MODULE_ID = "com.sickicarus.monitorbox.unifi"
MODULE_VERSION = "1.0.6"
MODULE_BUILD = 7
IMPORT_PACKAGE = "monitorbox_unifi_b7"
FILENAME = f"{MODULE_ID}-{MODULE_VERSION}-build{MODULE_BUILD}.zip"
BUILD7_OVERRIDE_BLOBS = {
    "discovery.py": "6afbd8d1df40110c399c2cc7ab47df16f95f92af",
    "phase2_policy.py": "c9209bb5dd5be5badffef872d54542c8b69a29d2",
}


def _source_files(root: Path) -> dict[str, bytes]:
    result = dict(previous._source_files(root))
    delta = previous.previous.previous._verified_directory(
        root / "sources" / "unifi" / "1.0.6-build7",
        BUILD7_OVERRIDE_BLOBS,
        label="UniFi v1.0.6 build 7 physical-repair delta",
    )
    result.update(delta)
    expected = set(previous.previous.previous.BASE_SOURCE_BLOBS) | {"phase2_policy.py"}
    if set(result) != expected:
        raise SystemExit("UniFi build 7 composed source shape changed")
    return result
...
def build(root: Path, output_dir: Path) -> Path:
    ...
    historical = set(previous.previous.previous.HISTORICAL_FILENAMES) | {
