#!/usr/bin/env python3
"""Build isolated #220 UI 1.4.0 build39 directly from accepted 1.3.1 build35.
...
TARGET_IMPORT_PACKAGE = "monitorbox_ui_b39"

RELEASE39 = stable.Release(
    build=UI_BUILD,
    certified_sha="p1-220-clean-card-projection",
    version=UI_VERSION,
)
SOURCE_BLOBS = {
...
def _replace_once(payload: bytes, old: bytes, new: bytes, label: str) -> bytes:
    if payload.count(old) != 1:
        raise SystemExit(f"UI build39 {label} seam changed: {old[:160]!r}")
    return payload.replace(old, new, 1)


def _source(root: Path) -> bytes:
    base = root / "sources" / "ui" / "1.4.0-build39"
    actual = {item.name for item in base.iterdir() if item.is_file()}
    expected = set(SOURCE_BLOBS)
    if actual != expected:
        raise SystemExit(
            f"UI build39 source shape changed: missing={sorted(expected-actual)} "
    ...
    if digest != SOURCE_BLOBS["card-projection.js"]:
        raise SystemExit(
            "UI build39 source drift: expected "
...
def _application(parent: bytes) -> bytes:
    payload = _replace_once(
        ...
        b"Standalone managed MonitorBox UI 1.4.0 build 39.",
        "standalone identity",
    )
    if PARENT_GENERATION.encode() not in payload:
        raise SystemExit("UI build39 parent missing build35 identity")
...
def _package_files(root: Path) -> dict[str, bytes]:
    ...
    for forbidden in (b"unifi", b"scrypted", b"portainer", b"nut", b"docker"):
        if forbidden in lowered:
            raise SystemExit("UI build39 contains provider/product coupling: "
                             + forbidden.decode())
    for token in (
        b"site?.cards",
        b"dashboard_card",
        ...
        if token not in card_js:
            raise SystemExit(f"UI build39 missing card contract {token!r}")
    if b"card_layout" in card_js or b"dashboardConfiguration" in card_js:
        raise SystemExit("UI build39 illegally imports #219 layout semantics")
    ...
    if b"aggregate-evidence.js" in parent["assets/dashboard.html"]:
        raise SystemExit("UI build39 must not import rejected build37 fallback")
    if parent["assets/dashboard.html"].count(script) != 1:
        raise SystemExit("UI build39 must load card projection exactly once")
    if any(PARENT_GENERATION.encode() in payload for payload in parent.values()):
        raise SystemExit("UI build39 retained stale build35 generation")
...
def build(root: Path, output_dir: Path) -> Path:
    ...
    target = output_dir / RELEASE39.filename
