#!/usr/bin/env python3
"""Idempotently stage UniFi Network v1.0.9 build 10 for #170."""
...
PREDECESSOR = ("com.sickicarus.monitorbox.unifi", "1.0.8", 9)
RELEASE = ("com.sickicarus.monitorbox.unifi", "1.0.9", 10)
ENTRY = {
    "manifest": {
        ...
        "entrypoints": {"integration": "monitorbox_unifi_b10:PLUGIN"},
    ...
    "package": "com.sickicarus.monitorbox.unifi-1.0.9-build10.zip",
}
...
def stage(path: Path) -> bool:
    ...
    if existing:
        if len(existing) != 1 or existing[0] != ENTRY:
            raise SystemExit(f"catalog release conflicts with #170 contract: {RELEASE}")
        print("UniFi 1.0.9 build 10 already staged")
        return False

    indexes = [index for index, item in enumerate(modules) if identity(item) == PREDECESSOR]
    if len(indexes) != 1:
        raise SystemExit(
            f"expected exactly one immutable UniFi predecessor {PREDECESSOR}, found {len(indexes)}"
        )
    modules.insert(indexes[0] + 1, ENTRY)
    path.write_text(json.dumps(source, separators=(",", ":")) + "\n", encoding="utf-8")
    print("staged UniFi Network 1.0.9 build 10")
    return True
