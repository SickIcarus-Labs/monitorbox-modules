...
ENTRY={
  "manifest":{
    ...
    "state_schema":1,
    "module_type":"ui",
    "entrypoints":{"webui":"monitorbox_ui_b25:install"},
...
def stage(path:Path)->bool:
    ...
    if existing:
        if len(existing)!=1 or existing[0]!=ENTRY: raise SystemExit(f"catalog release conflicts with P2 Phase-1 build25 contract: {RELEASE}")
        print("P2 Phase-1 UI build25 candidate already staged"); return False
