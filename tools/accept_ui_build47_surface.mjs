#!/usr/bin/env node
// UI47 #94: live public telemetry and inline renderer remain display-only.
import assert from 'node:assert/strict';
import fs from 'node:fs';
import vm from 'node:vm';

const base=new URL('../sources/ui/1.11.0-build47/',import.meta.url);
...
assert(html.startsWith('<button type="button" class="mb-card-shell"'),
  'All composed readings belong to one native button');
assert.equal((html.match(/data-object="a2"/g)||[]).length,1,
  'bindCards must find the canonical host exactly once');
assert(html.includes('<div class="parity-card"'),
  'The inherited header becomes static content, not a nested button');
assert(!html.includes('data-mb-source-object='),
  'Ordinary host rows must not expose individual drilldown controls');
assert(!html.includes('<button class="mb-card-item'),
  'Never nest interactive buttons inside the composed card');
...
assert(css.includes('.mb-card-native>.mb-card-item:first-child{border:0;}'));
assert(css.includes('button.mb-card-shell:focus-visible'));
assert(css.includes('grid-template-areas:none;grid-auto-flow:row'));
assert(css.includes('padding:2px 0;margin:0;border:0'));
assert(css.includes('font-family:inherit;font-size:12px'),
  'Added metrics use the same typography as the canonical dashboard');

assert.equal(JSON.stringify(site),before,'Telemetry changes cannot mutate source monitoring');

console.log('UI47: site-scoped 1 Hz gauge/rate, link-vs-scale basis, network status migration, fresh/stale/missing and inline rendering: PASS');
