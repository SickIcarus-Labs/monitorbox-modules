#!/usr/bin/env python3
"""#219 exact candidate: real Core state, installed managed UI47, Chromium and revisions.
...
import build_first_party_ui_build47 as ui_builder  # noqa:E402
...
def site_snapshot():
    ...
    site=SiteConfig(
        ...
        objects=tuple(objects),checks=tuple(checks),generation="paired-ui47",
    )
...
def archive():
    ...
    meta={**ui_stage.ENTRY["manifest"],"version":"1.11.0","build":47,
          "entrypoints":{"webui":"monitorbox_ui_b47:install"}}
    ...
    artifact=ManagedArtifact(
        ...
        artifact_filename=ui_builder.RELEASE47.filename,
    )
    ...
    def start(self):
        ...
        def run():
            ...
            async def boot():
                ...
                assert installed.active.manifest.build==47
                app=web.Application()
                ...
                assert (activation.version,activation.build)==("1.11.0",47)
                assert any(
...
def _save(page):
    ...
    try:
        ...
        raise AssertionError(
            "UI47 validation: "+page.locator("#layoutStatus").inner_text()
        ) from error
    ...
    with sync_playwright() as pw:
        ...
        try:
            for width,height in ((820,1050),(1366,900)):
                ...
                _choose(page,"CPU live","CPU live")
                page.locator("#donePicker").click()
                assert page.locator("#contentSelectedItems .mb-selected-item").count()==3
                _save(page)
                first=store.load()
                ...
                assert len(host_row["presentation"]["items"])==3
                ...
                _choose(page,"Pool live","Pool live")
                page.locator("#donePicker").click()
                assert page.locator("#contentSelectedItems .mb-selected-item").count()==5
                _save(page)
                second=store.load()
                ...
                )].filter(node=>node.getAttribute('data-mb-composer-key')
                    .includes('"live"')).length>=2""",timeout=10000)
                page.wait_for_timeout(1500)
                paired=[m for m in server.interest_messages
                    if {"lab/arrrrr2","lab/goliath"}.issubset(
                        set(m.get("details") or []))]
                assert paired,server.interest_messages[-8:]
                assert paired[-1]["detail"] in {
                    "lab/arrrrr2","lab/goliath"
                },paired[-1]
                ...
                context.close()
                print(f"UI47 real Core/managed UI + observed cross-provider metrics "
            ...
            # Historical UI47-v2 rows, including hidden Network members,
            ...
            context.close()
            print("UI47 exact historical UI47-v2 migration and rollback: PASS")
        finally:
            browser.close()
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="monitorbox-ui47-paired-") as temp:
        root=Path(temp)
        prepare(root)
    ...
    print("UI47 exact source sha256="+digest)
