#!/usr/bin/env python3
"""Build #353/#220 MonitorBox UI v1.3.2 build 36."""
...
TARGET_IMPORT_PACKAGE = "monitorbox_ui_b36"
RELEASE36 = stable.Release(
    build=UI_BUILD,
    certified_sha="p1-353-module-remove-p2-220-aggregate-fallback",
    version=UI_VERSION,
)
SOURCE_BLOBS = {
...
def _replace_once(payload: bytes, old: bytes, new: bytes, seam: str) -> bytes:
    if payload.count(old) != 1:
        raise SystemExit(f"UI build-36 {seam} seam changed: {old[:180]!r}")
...
def _delta_files(root: Path) -> dict[str, bytes]:
    ...
    for name, expected_blob in SOURCE_BLOBS.items():
        ...
        if actual_blob != expected_blob:
            raise SystemExit(
                f"UI build-36 source drift for {name}: expected {expected_blob}, got {actual_blob}"
            )
        result[name] = payload
    return result
...
def _application(parent: bytes) -> bytes:
    ...
    if PARENT_GENERATION.encode() not in payload:
        raise SystemExit("UI build-36 parent application has no build-35 generation marker")
...
def _package_files(root: Path) -> dict[str, bytes]:
    ...
    for token in (b"unifi", b"scrypted", b"portainer", b"nut"):
        if token in combined_js:
            raise SystemExit(
                "UI build36 contains provider-specific removal/aggregate coupling: "
                + token.decode("utf-8")
            )
    ...
    for token in (b"object.id==='network'", b'object.id==="network"', b"object.id==='cameras'", b"object.id==='power'"):
        if token in aggregate_lower:
            raise SystemExit("UI build36 aggregate fallback contains aggregate-name dispatch")

    if any(PARENT_GENERATION.encode() in payload for payload in parent.values()):
        raise SystemExit("UI build36 package retained build35 generation identity")
...
def build(root: Path, output_dir: Path) -> Path:
    ...
    target = output_dir / RELEASE36.filename
