#!/usr/bin/env python3
"""Build UniFi Network v1.0.5 build 6 over immutable build-5 dev history."""
...
import build_first_party_unifi_b5 as previous

MODULE_ID = "com.sickicarus.monitorbox.unifi"
MODULE_VERSION = "1.0.5"
MODULE_BUILD = 6
IMPORT_PACKAGE = "monitorbox_unifi_b6"
FILENAME = f"{MODULE_ID}-{MODULE_VERSION}-build{MODULE_BUILD}.zip"
BUILD6_OVERRIDE_BLOBS = {
    "phase2_policy.py": "b1652fda93c734c1b39cfb168cf261a72dd38fbd",
}


def _source_files(root: Path) -> dict[str, bytes]:
    result = dict(previous._source_files(root))
    delta = previous.previous._verified_directory(
        root / "sources" / "unifi" / "1.0.5-build6",
        BUILD6_OVERRIDE_BLOBS,
        label="UniFi v1.0.5 build 6 physical-repair delta",
    )
    result.update(delta)
    expected = set(previous.previous.BASE_SOURCE_BLOBS) | {"phase2_policy.py"}
    if set(result) != expected:
        raise SystemExit("UniFi build 6 composed source shape changed")
    return result


def _rewrite_source(name: str, payload: bytes) -> bytes:
    text = payload.decode("utf-8")
    for old, new in previous.previous._CORE_IMPORT_REWRITES:
        text = text.replace(old, new)

    entrypoint_count = text.count(previous.previous._BUNDLED_ENTRYPOINT)
    requirement_count = text.count(previous.previous._OLD_CORE_REQUIREMENT)
    if name in {"__init__.py", "runtime.py"}:
        ...
        text = text.replace(previous.previous._BUNDLED_ENTRYPOINT, managed_entrypoint, 1)
        text = text.replace(
            previous.previous._OLD_CORE_REQUIREMENT,
            previous.previous._NEW_CORE_REQUIREMENT,
            1,
        )
...
def build(root: Path, output_dir: Path) -> Path:
    ...
    print(
        ...
        f"build6_overrides={len(BUILD6_OVERRIDE_BLOBS)} entrypoint={IMPORT_PACKAGE}:PLUGIN"
    )
