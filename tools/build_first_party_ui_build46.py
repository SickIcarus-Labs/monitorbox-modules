#!/usr/bin/env python3
"""UI46 #94: inline live dashboard readings layered over immutable signed UI43."""
...
import build_first_party_ui_build45 as previous
...
TARGET_IMPORT_PACKAGE = "monitorbox_ui_b46"
RELEASE46 = stable.Release(
    build=UI_BUILD, certified_sha="feature-94-ui46-layout-live-repair", version=UI_VERSION,
)
SOURCE_BLOBS = {
    "card-composer-renderer.js": "bdd51cd8a432763c0bdb607b14ad9626f8050f00",
    "card-composer.css": "ec21d40e924fb9858ab48829ebe165159e9b3925",
    ...
    for name in ("card-item-registry.js", "card-layout-policy.js",
        ...
        for provider in (b"unifi", b"scrypted", b"portainer", b"meraki", b"eero"):
            if provider in source[name].lower():
                raise SystemExit(f"UI46 shared composer contains provider-specific logic: {name}")
    return source
...
def _package_files(root: Path) -> dict[str, bytes]:
    ...
    if any(html.count(path) != 1 for path in required):
        raise SystemExit("UI46 homepage is missing or duplicating an asset")
    if not (html.index(required[0]) < html.index(required[1])
            < html.index(required[2]) < html.index(required[3])):
        raise SystemExit("UI46 deferred script dependency order changed")
    if parent["assets/card-layout-editor.html"].count(b'itemPicker') != 1:
        raise SystemExit("UI46 shared item picker disappeared")
    if inherited[prefix + "assets/card-projection.js"] != parent["assets/card-projection.js"]:
        raise SystemExit("UI46 changed accepted canonical Core card projection")
    if PARENT_GENERATION.encode() in html:
        raise SystemExit("UI46 homepage contains stale UI43 script identities")
...
def build(root: Path, output_dir: Path) -> Path:
    ...
    target = output_dir / RELEASE46.filename
    target.write_bytes(data)
    print(f"UI46 dev candidate {target}: sha256={hashlib.sha256(data).hexdigest()}")
    return target
