#!/usr/bin/env python3
"""#219 exact candidate: real Core state, installed managed UI44, Chromium and revisions.
...
import build_first_party_ui_build44 as ui_builder  # noqa:E402
...
def site_snapshot():
    ...
    site=SiteConfig(
        ...
        objects=tuple(objects),checks=tuple(checks),generation="paired-ui44",
    )
...
def archive():
    ...
    meta={**ui_stage.ENTRY["manifest"],"version":"1.8.0","build":44,
          "entrypoints":{"webui":"monitorbox_ui_b44:install"}}
    ...
    artifact=ManagedArtifact(
        ...
        artifact_filename=ui_builder.RELEASE44.filename,
    )
    ...
    def start(self):
        ...
        def run():
            ...
            async def boot():
                ...
                assert installed.active.manifest.build==44
                app=web.Application()
                ...
                assert (activation.version,activation.build)==("1.8.0",44)
                assert any(
...
def _save(page):
    ...
    try:
        ...
        raise AssertionError(
            "UI44 validation: "+page.locator("#layoutStatus").inner_text()
        ) from error
    ...
    with sync_playwright() as pw:
        ...
        try:
            for width,height in ((820,1050),(1366,900)):
                ...
                context.close()
                print(f"UI44 real Core/managed UI + observed cross-provider metrics "
            ...
            # Historical UI44-v2 rows, including hidden Network members,
            ...
            context.close()
            print("UI44 exact historical UI44-v2 migration and rollback: PASS")
        finally:
            browser.close()
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="monitorbox-ui44-paired-") as temp:
        root=Path(temp)
        prepare(root)
    ...
    print("UI44 exact source sha256="+digest)
