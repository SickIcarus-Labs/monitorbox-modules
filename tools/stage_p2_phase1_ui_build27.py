#!/usr/bin/env python3
"""Idempotently stage the P2 Phase-1 UI v1.1.14 build-27 icon-correction candidate."""
...
RELEASE=("com.sickicarus.monitorbox.ui","1.1.14",27)
ENTRY={
  "manifest":{
    ...
    "state_schema":1,
    "module_type":"ui",
    "entrypoints":{"webui":"monitorbox_ui_b27:install"},
  ...
  "package":"com.sickicarus.monitorbox.ui-1.1.14-build27.zip"
}
...
def stage(path:Path)->bool:
    ...
    if existing:
        if len(existing)!=1 or existing[0]!=ENTRY: raise SystemExit(f"catalog release conflicts with P2 Phase-1 build27 contract: {RELEASE}")
        print("P2 Phase-1 UI build27 candidate already staged"); return False
    ...
    print("staged P2 Phase-1 UI 1.1.14 build 27 after accepted build 23; rejected/working builds24/25/26 remain out of catalog history")
    return True
