#!/usr/bin/env python3
"""Disposable UI48 iPad/desktop picker and genuine one-second DOM repaint tests."""
...
async def editor_accept(browser,base,fixture,width,height):
    ...
    await context.close()
    print(f"UI48 live grouped picker, iPad/desktop v2→v3 and full A/B selected-item restoration: PASS {width}x{height}")
...
async def live_card_accept(browser,width,height):
    ...
    assert not errors,errors
    print(f"UI48 iPad/desktop 24-item natural-height columns, exact NIC, "
