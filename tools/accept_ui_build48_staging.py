#!/usr/bin/env python3
"""UI48 additive staged release and exact UI42 immutable signed predecessor."""
...
def main()->None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert all(row["manifest"]["build"]!=47
                   for row in result
                   if row["manifest"]["module_id"]==MODULE)
        assert stage.stage(path) is False
        assert json.loads(path.read_text())["modules"]==result
    print("UI48: exact accepted UI42 digest, unchanged cumulative source, "
          "one staged UI48 dev supersession and idempotent staging: PASS")
