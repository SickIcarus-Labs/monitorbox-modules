#!/usr/bin/env python3
"""Idempotently stage the SNMP 1.0.4/build-5 zipimport hotfix release."""
...
PREVIOUS = (MODULE_ID, "1.0.3", 4)
HOTFIX = (MODULE_ID, "1.0.4", 5)
HOTFIX_ENTRY = {
    "manifest": {
        "module_id": MODULE_ID,
        ...
        "entrypoints": {"integration": "monitorbox_snmp_b5:PLUGIN"},
    ...
    "package": "com.sickicarus.monitorbox.snmp-1.0.4-build5.zip",
}
...
def stage(path: Path) -> bool:
    ...
    if existing:
        if len(existing) != 1 or existing[0] != HOTFIX_ENTRY:
            raise SystemExit("SNMP 1.0.4 build 5 catalog entry conflicts with the hotfix contract")
        print("SNMP 1.0.4 build 5 already staged")
        return False

    previous_indexes = [index for index, item in enumerate(modules) if identity(item) == PREVIOUS]
    if len(previous_indexes) != 1:
        raise SystemExit(f"expected exactly one SNMP 1.0.3 build 4 release, found {len(previous_indexes)}")
    modules.insert(previous_indexes[0] + 1, HOTFIX_ENTRY)
    path.write_text(json.dumps(source, separators=(",", ":")) + "\n", encoding="utf-8")
    print("staged SNMP 1.0.4 build 5 after immutable build 4")
    return True
