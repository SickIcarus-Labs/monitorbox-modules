#!/usr/bin/env python3
"""UI45 #94: inline live dashboard readings layered over immutable signed UI43."""
...
TARGET_IMPORT_PACKAGE = "monitorbox_ui_b45"
RELEASE45 = stable.Release(
    build=UI_BUILD, certified_sha="feature-94-ui45-layout-live-repair", version=UI_VERSION,
)
SOURCE_BLOBS = {
    "card-composer-renderer.js": "bdc0b48acabbdbaa6e7338de306abf86d93fd09b",
    "card-composer.css": "5822f3e2cc4f19d33dc3f60afd2dd23521b12583",
    ...
    for name in ("card-item-registry.js", "card-layout-policy.js",
        ...
        for provider in (b"unifi", b"scrypted", b"portainer", b"meraki", b"eero"):
            if provider in source[name].lower():
                raise SystemExit(f"UI45 shared composer contains provider-specific logic: {name}")
    return source
...
def _package_files(root: Path) -> dict[str, bytes]:
    ...
    if any(html.count(path) != 1 for path in required):
        raise SystemExit("UI45 homepage is missing or duplicating an asset")
    if not (html.index(required[0]) < html.index(required[1])
            < html.index(required[2]) < html.index(required[3])):
        raise SystemExit("UI45 deferred script dependency order changed")
    if parent["assets/card-layout-editor.html"].count(b'itemPicker') != 1:
        raise SystemExit("UI45 shared item picker disappeared")
    if inherited[prefix + "assets/card-projection.js"] != parent["assets/card-projection.js"]:
        raise SystemExit("UI45 changed accepted canonical Core card projection")
    if PARENT_GENERATION.encode() in html:
        raise SystemExit("UI45 homepage contains stale UI43 script identities")
...
def build(root: Path, output_dir: Path) -> Path:
    ...
    target = output_dir / RELEASE45.filename
    target.write_bytes(data)
    print(f"UI45 dev candidate {target}: sha256={hashlib.sha256(data).hexdigest()}")
    return target
