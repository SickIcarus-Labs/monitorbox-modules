#!/usr/bin/env python3
"""Stage the Phase-5 SNMP 1.0.6 build-8 release identity."""
...
PREDECESSOR = (MODULE_ID, "1.0.5", 7)
RELEASE = (MODULE_ID, "1.0.6", 8)
ENTRY = {
    "manifest": {
        "module_id": MODULE_ID,
        ...
        "entrypoints": {"integration": "monitorbox_snmp_v106_b8:PLUGIN"},
    ...
    "package": "com.sickicarus.monitorbox.snmp-1.0.6-build8.zip",
}
...
def stage(path: Path) -> bool:
    ...
    if existing:
        if len(existing) != 1 or existing[0] != ENTRY:
            raise SystemExit(f"catalog release conflicts with Phase-5 contract: {RELEASE}")
        print("SNMP 1.0.6 build 8 already staged")
        return False

    indexes = [index for index, item in enumerate(modules) if identity(item) == PREDECESSOR]
    if len(indexes) != 1:
        raise SystemExit(
            f"expected exactly one immutable SNMP predecessor {PREDECESSOR}, found {len(indexes)}"
        )
    modules.insert(indexes[0] + 1, ENTRY)
    path.write_text(json.dumps(source, separators=(",", ":")) + "\n", encoding="utf-8")
    print("staged SNMP 1.0.6 build 8 after immutable build 7")
    return True
