#!/usr/bin/env python3
"""UI45 additive staged release and exact UI42 immutable signed predecessor."""
...
import stage_94_ui_build45 as stage
...
def main()->None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert all(row["manifest"]["build"]!=44
                   for row in result
                   if row["manifest"]["module_id"]==MODULE)
        assert stage.stage(path) is False
        assert json.loads(path.read_text())["modules"]==result
    print("UI45: exact accepted UI42 digest, unchanged cumulative source, "
          "one staged UI45 dev supersession and idempotent staging: PASS")
