#!/usr/bin/env python3
"""Build the independently managed SNMP integration for MonitorBox 2.3 Core."""
...
MODULE_BUILD = 1
IMPORT_PACKAGE = "monitorbox_snmp_b1"
...
def _source_files(root: Path) -> dict[str, bytes]:
    source_root = root / "sources" / "snmp" / "1.0.0-build1"
    ...
    if actual_names != expected_names:
        ...
        raise SystemExit(
            f"SNMP 1.0.0 build 1 source shape changed: missing={missing}, extra={extra}"
        )
    ...
    for name, expected_blob in SOURCE_BLOBS.items():
        ...
        if actual_blob != expected_blob:
            raise SystemExit(
                f"SNMP 1.0.0 build 1 source drift for {name}: "
...
def _rewrite_source(name: str, payload: bytes) -> bytes:
    ...
    for old, new in _CORE_IMPORT_REWRITES:
        if text.count(old) != 1:
            raise SystemExit(f"SNMP managed import contract changed for {old!r} in {name}")
