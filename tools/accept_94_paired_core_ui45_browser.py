#!/usr/bin/env python3
"""#219 exact candidate: real Core state, installed managed UI45, Chromium and revisions.
...
import build_first_party_ui_build45 as ui_builder  # noqa:E402
...
def site_snapshot():
    ...
    site=SiteConfig(
        ...
        objects=tuple(objects),checks=tuple(checks),generation="paired-ui45",
    )
...
def archive():
    ...
    meta={**ui_stage.ENTRY["manifest"],"version":"1.9.0","build":45,
          "entrypoints":{"webui":"monitorbox_ui_b45:install"}}
    ...
    artifact=ManagedArtifact(
        ...
        artifact_filename=ui_builder.RELEASE45.filename,
    )
    ...
    def start(self):
        ...
        def run():
            ...
            async def boot():
                ...
                assert installed.active.manifest.build==45
                app=web.Application()
                ...
                assert (activation.version,activation.build)==("1.9.0",45)
                assert any(
...
def _save(page):
    ...
    try:
        ...
        raise AssertionError(
            "UI45 validation: "+page.locator("#layoutStatus").inner_text()
        ) from error
    ...
    with sync_playwright() as pw:
        ...
        try:
            for width,height in ((820,1050),(1366,900)):
                ...
                context.close()
                print(f"UI45 real Core/managed UI + observed cross-provider metrics "
            ...
            # Historical UI45-v2 rows, including hidden Network members,
            ...
            context.close()
            print("UI45 exact historical UI45-v2 migration and rollback: PASS")
        finally:
            browser.close()
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="monitorbox-ui45-paired-") as temp:
        root=Path(temp)
        prepare(root)
    ...
    print("UI45 exact source sha256="+digest)
