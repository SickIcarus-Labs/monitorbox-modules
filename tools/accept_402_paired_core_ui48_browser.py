#!/usr/bin/env python3
"""#219 exact candidate: real Core state, installed managed UI48, Chromium and revisions.
...
import build_first_party_ui_build48 as ui_builder  # noqa:E402
...
def archive():
    ...
    meta={**ui_stage.ENTRY["manifest"],"version":"1.12.0","build":48,
          "entrypoints":{"webui":"monitorbox_ui_b48:install"}}
    ...
    artifact=ManagedArtifact(
        ...
        artifact_filename=ui_builder.RELEASE48.filename,
    )
    ...
    def start(self):
        ...
        def run():
            ...
            async def boot():
                ...
                assert installed.active.manifest.build==48
                app=web.Application()
                ...
                assert (activation.version,activation.build)==("1.12.0",48)
                assert any(
    ...
    with sync_playwright() as pw:
        ...
        try:
            for width,height in ((820,1050),(1366,900)):
                ...
                context.close()
                print(f"UI48 real Core/managed UI + observed cross-provider metrics "
            ...
            # Historical UI48-v2 rows, including hidden Network members,
            ...
            context.close()
            print("UI48 exact historical UI48-v2 migration and rollback: PASS")
        finally:
            browser.close()
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="monitorbox-ui48-paired-") as temp:
        root=Path(temp)
        prepare(root)
    ...
    print("UI48 exact source sha256="+digest)
