#!/usr/bin/env python3
"""Stage exactly one UI48 dev release over immutable UI42 main source history.

UI47 is already signed in official-dev. Its authority is verified via the
release intent's supersedes_dev proof; do not add or modify UI47 in main.
"""
...
ENTRY["package"]="com.sickicarus.monitorbox.ui-1.12.0-build48.zip"
...
def stage(path:Path)->bool:
    ...
    if not isinstance(entries,list):
        raise SystemExit("UI48 requires cumulative signed-beta source catalog")
    existing=[row for row in entries if identity(row)==RELEASE]
    if existing:
        if len(existing)!=1 or existing[0]!=ENTRY:
            raise SystemExit("Conflicting immutable UI48 release identity")
        return False
    ...
    if len(predecessor)!=1 or entries[predecessor[0]]!=UI42_ENTRY:
        raise SystemExit("UI48 requires exact signed-beta UI42 source row")
    ...
    print("Staged one UI 1.11.0 build47 release; signed UI47 is dev-only predecessor")
    return True
