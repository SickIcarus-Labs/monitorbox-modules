#!/usr/bin/env python3
"""Acceptance for self-contained managed SNMP 1.0.3 build 4."""
...
PACKAGE_NAME = "com.sickicarus.monitorbox.snmp-1.0.3-build4.zip"
IMPORT_PACKAGE = "monitorbox_snmp_b4"
COMMUNITY = "snmp-build4-community-secret"
USERNAME = "snmp-build4-v3-user"
AUTH_PASSWORD = "snmp-build4-auth-secret"
PRIVACY_PASSWORD = "snmp-build4-privacy-secret"
...
async def accept() -> None:
    ...
    if not package.is_file():
        raise AssertionError(f"managed SNMP build 4 package is missing: {package}")

    with zipfile.ZipFile(package) as archive:
        names = set(archive.namelist())
        if not any(name.startswith("pysnmp/") for name in names):
            raise AssertionError("SNMP build 4 did not vendor PySNMP")
        if not any(name.startswith("pyasn1/") for name in names):
            raise AssertionError("SNMP build 4 did not vendor PyASN1")
    ...
    for forbidden in ("snmpget", "create_subprocess_exec", "monitorbox.v2.integrations.snmp"):
        if forbidden in root_text + runtime_text:
            raise AssertionError(f"SNMP build 4 retained hidden Core/OS transport dependency: {forbidden}")
    ...
    if (managed.MODULE_VERSION, managed.MODULE_BUILD) != ("1.0.3", 4):
        raise AssertionError("SNMP build 4 release identity changed")
    if managed.MODULE_MANIFEST.entrypoints != {"integration": f"{IMPORT_PACKAGE}:PLUGIN"}:
        raise AssertionError("SNMP build 4 entrypoint is not generation-safe")
    if managed.MODULE_MANIFEST.requires_core != ">=2.3.1 <3.0.0":
        raise AssertionError("SNMP build 4 Core floor changed")
    if managed.PLUGIN.runtime_executor is None or managed.PLUGIN.runtime_adapter_kinds != ("snmp",):
        raise AssertionError("SNMP build 4 does not own the snmp runtime adapter")

    # Import the vendored transport from the module ZIP and exercise v3 credential
    # construction without putting passphrases on an external process command line.
    ...
    if credentials is None or set(protected) != {USERNAME, AUTH_PASSWORD, PRIVACY_PASSWORD}:
        raise AssertionError("SNMPv3 build 4 credential projection changed")

    executor = managed.PLUGIN.runtime_executor
    context = plugin_api.RuntimeExecutionContext(
        module_id=managed.MODULE_ID,
        package_root=str(package),
        state_root="/tmp/snmp-build4-state",
    )
    # The real engine was already proven importable above. Synthetic query fixtures
    # isolate state semantics from network availability.
    ...
    if healthy.state != "healthy" or healthy.metrics.get("uptime") != 42.0:
        raise AssertionError(f"SNMP build 4 did not return healthy numeric metrics: {healthy!r}")
    if healthy.metadata.get("transport") != "pysnmp":
        raise AssertionError("SNMP build 4 lost explicit PySNMP transport provenance")
    ...
    print("managed SNMP 1.0.3 build 4 pruned-Core runtime acceptance: PASS", flush=True)
